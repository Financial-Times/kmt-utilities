...
function _authenticationError(err, req, res, next) {
  ...
  return fetch(config.LOGOUT_URL, {headers: req.headers, mothod: 'post'})
    .then(() => {
      return res.redirect(uriConstructor.redirectUrl(req));
    });
}

function middleware(err, req, res, next) {
  if (err.status === 401) {
    _authenticationError(err, req, res, next);
  } else {
    _applicationError(err, req, res, next);
  }
}
