...
module.exports = function* (req, res, next) {
  ...
  //Call apiAuth service here
    try {
      req.apiAuthToken = req.apiAuthToken || (yield apiAuthService.getAuthToken(FTSessionSecure));

      logger.info({operation:'getAuthToken', result:JSON.stringify(req.apiAuthToken), licenceId:req.licenceId});
      next();
    } catch (err) {
