...
function* _licenceDetails(licenceId, adminUserId, apiAuthToken) {
  const responses = yield [
      ...
      licenceDataService.getAdminUserList(licenceId, adminUserId, apiAuthToken)
    ];
  return responses;
}
...
function setKATConfig(req, licenceList) {
    ...
    var KATConfig = Object.assign({}, req.session.kmtLoggedIn, {licenceList:licenceList});
    
    KATConfig.activeLicence = licenceList.find(licence => licence.licenceId === req.params.licenceId);
  ...
  logger.info({operation:'isAdminUser', licence: req.licenceId, user: currentUser});
    ...
    try {
      ...
      if (req.session.isPopulated && Array.isArray(licenceList) && isLicenceInList(licenceList, req.licenceId)) {
          ...
          logger.info({operation:'isAdminUser', sessDetails:JSON.stringify(req.session.kmtLoggedIn)});
