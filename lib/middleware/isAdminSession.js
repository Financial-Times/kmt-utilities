...
function* isAdminUser(req, res, next) {
  ...
  logger.info({operation:'isAdminUser', licence: req.licenceId});
    ...
    try {
      ...
      logger.info({operation:'DoesKMTsessions exist', user: req.currentUser.uuid});
      logger.info({operation:'DoesKMThaveLicences', kmtLicences: cookieHandler.getLicenceListCookie(req, res)});
      //Todo should we check for FTSession here too?
      if (req.session.isPopulated && res.cookie.KATLicences) {
          logger.info({operation:'hasKMTSession', kmtSessionExists: 'true', sessionValue: cookieHandler.getKatCookie(req, res)});
          logger.info({operation:'isAdminUser', licenceExistsInSession:true});
          logger.info({operation:'licenceListFresh', licenceListFresh:true});
          next();
        ...
        req.KATConfig = Object.assign({}, req.session.kmtLoggedIn,{licenceList:listOfLicences});

        //set KATLicence list cookie
        res.cookie('KATLicences', JSON.stringify(listOfLicences), { expires: new Date(Date.now() + 2 * 60 * 1000)});

        logger.info({operation:'KMTSessionSet', forLicence:req.params.licenceId, loggedInLicenceListSet:true});
        logger.info({operation:'kmtConfig', kmtConfigObj:JSON.stringify(req.kmtConfig)});
        next();
