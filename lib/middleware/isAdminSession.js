...
function* isAdminUser(req, res, next) {
  ...
  logger.info({operation:'isAdminUser', reqPath:req.path, user:JSON.stringify(req.currentUser), licence: req.licenceId});

    try {
      //Does a KMT session exist
      if (req.session.isPopulated) {
          logger.info({operation:'isAdminUser', sessionUser:JSON.stringify(req.session.adminUser)});
          next();
