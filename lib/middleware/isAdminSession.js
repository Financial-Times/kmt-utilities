...
function _cacheLicenceList(req, res, licenceList) {
  logger.log('info', {operation:'cacheLicenceList', licenceId:req.licenceId});
  //set KATLicence list cookie
  res.cookie('FTKatL', JSON.stringify(licenceList), { expires: new Date(Date.now() + 10 * 60 * 1000)});
}

function setKATConfig(req, licenceList) {
    logger.info({operation:'KATConfigSet', KATConfig:JSON.stringify(req.KATConfig)});
...
function* isAdminUser(req, res, next) {
  ...
  logger.info({operation:'isAdminUser', licence: req.licenceId, user: currentUser});
    ...
    try {
      ...
      _cacheLicenceList(req, res, licenceList);
      next();
