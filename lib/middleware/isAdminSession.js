...
function* isAdminUser(req, res, next) {
  ...
  logger.info({operation:'isAdminUser', licence: req.licenceId});
    ...
    try {
      ...
      if (req.session.isPopulated && res.cookie.FTKatL) {
          ...
          logger.info({operation:'licenceListFresh', licenceListFresh:res.cookies.FTKatL});

      } else if (req.session.isPopulated && !res.cookie.FTKatL) {
