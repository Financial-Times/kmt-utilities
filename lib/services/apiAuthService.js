...
const redirectUrl = 'https://kmt-staging.ft.com/echo';
...
module.exports = {
  getAuthToken: function* (FTSessionSecure) {
    ...
    logger.info({operation:'getAuthToken', requestUri:serviceUri + `/authorize?response_type=token&client_id=${clientId}`, Headers:JSON.stringify(options.headers)});
    ...
    try {
      ...
      const response = yield fetch(serviceUri + `/authorize?response_type=token&client_id=${clientId}&redirect_uri=${redirectUrl}`, options);
        if (response.status !== 200) {
          //TODO redirectUrl is causing a 404
      ...
      const authObj = uriFragSplitter(response.url);
       ...
       //Happy path
         if ('access_token' in authObj) {
           ...
           return yield authObj.access_token;
