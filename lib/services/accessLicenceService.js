...
const accessLicenceServiceUrl = config.get('API_GATEWAY_HOST') + '/licences';
...
module.exports = {
  ...
  getLicenceInfo: function* (accessLicenceId) {
    ...
    if (response.status >= 400) {
      logger.info({operation:'getLicenceInfo', licenceId:accessLicenceId, url:`${accessLicenceServiceUrl}/${accessLicenceId}` ,result:'failed', error:response.statusText});
