...
module.exports = {
  ...
  getLicenceList: function* (options = null) {
    ...
    logger.log('info', {operation:'getLicenceList', request:endpoint});

    let response = yield fetch(endpoint, { headers: headers });

    if (response.status >= 400) {
      logger.log('info', {operation:'getLicenceList', filteredBy:JSON.stringify(options), result:'failed', error:response.statusText});
