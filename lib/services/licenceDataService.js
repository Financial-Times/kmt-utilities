...
function* getUserList(accessLicenceId, authToken) {

    headers.authorize = `Bearer ${authToken}`;
    logger.info({operation:'getUserList', requestUrl:licenceDataService +accessLicenceId, autToken:JSON.stringify(headers.authorize)});
...
function* getAdminUserList(accessLicenceId, adminUserId, apiAuthToken) {
  ...
  if (apiAuthToken) {
    headers.authorize = `Bearer ${apiAuthToken}`;
  }

  logger.info({operation:'getAdminUserList', requestUrl:licenceDataService +accessLicenceId+'/admins', headers:JSON.stringify({headers:headers})});
